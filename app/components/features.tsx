    ...
    <Section>
      <Container className="not-prose" dir="rtl">
        <div className="flex flex-col gap-6">
          ...
          <h4 className="text-2xl font-light opacity-70 text-right">
            <Balancer>
          ...
          <div>
            {singleFeatureText.map(
              ({ icon, title, description, href, cta }, index) => (
                <Link
                  href={`${href}`}
                  className="flex flex-col justify-between gap-6 rounded-lg border bg-muted/25 p-6 transition-all hover:-mt-2 hover:mb-2 text-right"
                  key={index}
                >
                  <div className="grid gap-4">
                    <div className="flex justify-end">{icon}</div>
